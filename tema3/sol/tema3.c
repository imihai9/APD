...
void c0_assign_arr_to_coords (int *c_cnt_workers, int *coords, int cnt_workers_total) {
...
void c_assign_arr_to_workers(int *v, int len, int cnt_workers) {
    ...
    for (int id = 0; id < cnt_workers; id++) {
        ...
        send_print(v + start, chunk_len, MPI_INT, workers[id], ARRAY_SEND_TAG, 1);
    }
}
...
void c_send_arr_to_c0(int *v_new, int len) {
    ...
    send_print(v_new, len, MPI_INT, 0, ARRAY_SEND_TAG, 1);
}
...
void exec_coordinator_part2 () {
    if (rank == 0) {
        ...
        c0_assign_arr_to_coords(c_cnt_workers, coords, cnt_workers_total);
    }
    else
